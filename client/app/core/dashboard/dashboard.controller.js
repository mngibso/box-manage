...
(function(){
  ...
  angular.module('manageBox.core.dashboard')
    .controller('manageBox.core.dashboard.MainCtrl',MainController);
    //['$scope'
      //,'$window'
  ...
  function MainController($scope) {
    console.log('dashbaord MainController')
    $scope.blah = function(){ console.log('blah'); } ;
    $scope.duh = function(){ alert('duh2');console.log('duh2'); } ;
    $scope.message= 'thisWorks';
  }
  function MainControllerx($scope, $window, socket, thing, box, auth, noty) {
    console.log('dashboard mainController');
    $scope.awesomeThings = [];
    $scope.boxDocuments = [];
    ...
    $scope.deleteThing = function(theThing) {
      ...
      if(theThing._id) thing.delete( theThing._id ).then(function(){ });
    };
